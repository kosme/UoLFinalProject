...
MMA8452Q::MMA8452Q(byte addr) {
  _deviceAddress = addr;
  // Store address into private variable
}
...
bool MMA8452Q::begin(TwoWire &wirePort, uint8_t deviceAddress) {
  ...
  byte c = readID(); // Read WHO_AM_I register
...
// INITIALIZATION
// This function initializes the MMA8452Q. It sets up the scale (either 2,
// 4, or 8g), output data rate, portrait/landscape detection and tap
// detection.
// It also checks the WHO_AM_I register to make sure we can communicate
// with 	the sensor. Returns a 0 if communication failed, 1 if
// successful.
byte MMA8452Q::init(MMA8452Q_Scale fsr, MMA8452Q_ODR odr) {
  ...
  setupPL();        // Set up portrait/landscape detection
  // Multiply parameter by 0.0625g to calculate threshold.
  setupTap(0x80, 0x80, 0x80); // Disable x, y, z
...
byte MMA8452Q::readID() { return readRegister(WHO_AM_I); }
...
//		  of the acceleromter.
//		* floats cx, cy, and cz will store the calculated acceleration
//      from those 12-bit values. These variables are in units of g's.
void MMA8452Q::read() {
  byte rawData[6]; // x/y/z accel register data stored here

  readRegisters(OUT_X_MSB, rawData,
                6); // Read the six raw data registers into data array
...
byte MMA8452Q::availableX() { return (readRegister(STATUS_MMA8452Q) & 0x01); }

byte MMA8452Q::availableY() {
  return (readRegister(STATUS_MMA8452Q) & 0x02) >> 1;
}

byte MMA8452Q::availableZ() {
  return (readRegister(STATUS_MMA8452Q) & 0x04) >> 2;
}
...
void MMA8452Q::startMotionDetection() {
  if (isActive() == true)
    standby();
  writeRegister(FF_MT_CFG, 0x48);
...
//		2. Set tap g's threshold. The lower 7 bits will set the tap
...
//	returns no new taps were detected. Otherwise the function will return
// the 	lower 7 bits of the PULSE_SRC register.
...
// MMA8452Q. 	It will return either PORTRAIT_U, PORTRAIT_D, LANDSCAPE_R,
// LANDSCAPE_L, 	or LOCKOUT. LOCKOUT indicates that the sensor is in
// neither p or ls.
...
// and 	auto-incrmenting to the next.
...
byte MMA8452Q::readRegister(MMA8452Q_Register reg) {
  ...
  _i2cPort->requestFrom((uint8_t)_deviceAddress, (uint8_t)1, (uint32_t)reg,
                        (uint8_t)1, true);
#else
  _i2cPort->beginTransmission(_deviceAddress);
  _i2cPort->write(reg);
  ...
  _i2cPort->requestFrom(
      _deviceAddress,
      (byte)1); // Ask for 1 byte, once done, bus is released by default
#endif
  if (_i2cPort->available()) { // Wait for the data to come back
...
//	Read "len" bytes from the MMA8452Q, starting at register "reg". Bytes
// are stored 	in "buffer" on exit.
void MMA8452Q::readRegisters(MMA8452Q_Register reg, byte *buffer, byte len) {
#ifdef _VARIANT_ARDUINO_DUE_X_
  _i2cPort->requestFrom((uint8_t)_deviceAddress, (uint8_t)len, (uint32_t)reg,
                        (uint8_t)1, true);
#else
  _i2cPort->beginTransmission(_deviceAddress);
  _i2cPort->write(reg);
  ...
  _i2cPort->requestFrom(
      _deviceAddress,
      len); // Ask for bytes, once done, bus is released by default
#endif
