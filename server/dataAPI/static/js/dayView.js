// Removing references to the server URL guarantees that the url for the /medical
// endpoint and the API requests is the same, therefore user credential exist on the
// user browser which is required for fetching data.
...
function gotoEpisodeView(episodeTimestamp, container) {
  const user = document.getElementById('patientSelector').value;
  if (document.getElementById('episodeViewContainer').classList.contains('hidden')) {
    ...
    xhr.onreadystatechange = function () {
      if (this.readyState == 4 && this.status == 200) {
        ...
        document.getElementById('episodeViewContainer').classList.remove('hidden');
        episodePlot(document.getElementById('episodeView'), data);
      }
    };
    xhr.open("GET", "/event/" + user + "?timestamp=" + episodeTimestamp / 1000);
...
function dateChanged() {
  var day = moment(document.getElementById('dayPicker').value);
...
function fetchDayData(data, start, stop) {
  ...
  xhr.open("GET", "/data/" + user + "?start=" + start + "&stop=" + stop);
  // xhr.withCredentials = true;
  xhr.responseType = "json";
  xhr.send();
}
